...
import {
  RESEARCH_SOURCES,
  COMMERCE_INTEL_SOURCES,
  MONITOR_POOL_LABELS,
  COMMERCE_INTEL_POOL_LABELS,
  researchSourcesByPool,
  commerceIntelSourcesByPool,
  type MonitorPool,
  type CommerceIntelPool,
...
const COMMERCE_POOL_ORDER = Object.keys(
  COMMERCE_INTEL_POOL_LABELS,
) as CommerceIntelPool[];
...
export default function SourcesPage() {
  ...
  const commerceByPool = commerceIntelSourcesByPool();
    ...
    <div className="container py-10 space-y-10">
      <div className="space-y-3">
        ...
        <p className="text-[var(--muted)] max-w-2xl leading-relaxed">
          本页审计「数据从哪来、写到哪、是否抓取成功」，并提供全球 AI 情报雷达与商业掘金信息源链接。
          本项目使用 JSON bundle，不伪造数据库连接串。
        </p>
        <div className="flex flex-wrap gap-2 text-xs">
          <span className="tag tag-signal">日更公开源 {LIVE_SOURCES.length} 路</span>
          <span className="tag">AI 监控池 {RESEARCH_SOURCES.length} 个</span>
          <span className="tag">商业掘金 {COMMERCE_INTEL_SOURCES.length} 个</span>
      ...
      <section className="space-y-4">
        <div className="space-y-2">
          <h2 className="display text-xl font-semibold">商业掘金情报源（可点击）</h2>
          <p className="text-sm text-[var(--muted)] max-w-3xl leading-relaxed">
            搜索趋势 / 跨境选品 / 资本行业 / 落地交付。与上方 AI 5 池并列展示，用于信息差与变现调研；
            <span className="tag ml-1">不进雷达 5 池</span>
            ，也不改七维分数。每日最小组合：Google Trends → Exploding Topics → FastMoss（跨境时）→
            CB Insights。
          </p>
        </div>
        <div className="space-y-6">
          {COMMERCE_POOL_ORDER.map((pool) => (
            <div key={pool} className="space-y-2">
              <h3 className="text-sm font-semibold text-[var(--text)]">
                {COMMERCE_INTEL_POOL_LABELS[pool]}
                <span className="ml-2 text-xs font-normal text-[var(--muted)]">
                  {commerceByPool[pool].length} 个
                </span>
              </h3>
              <div className="grid md:grid-cols-2 gap-2">
                {commerceByPool[pool].map((s) => (
                  <div key={s.id} className="surface px-4 py-3 space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <IconChip
                        icon={SOURCE_LEVEL_ICONS[s.level]}
