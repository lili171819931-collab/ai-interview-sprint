...
- 🌫️ **背景模糊优化**：`页面模糊`（小窗背后的网页内容虚化）/ `人像柔焦`（摄像头中心清晰、边缘虚化）
...
扩展能力：录屏（当前标签页 / 窗口 / 整个屏幕）、摄像头小窗（拖动 / 缩放 / 5 种形状 / 美颜 / 背景模糊）、
麦克风混音、字幕（手动 / Chrome AI 语音识别）、网页内点击特效、自动聚焦缩放；
录完自动下载，并一键跳回本页面继续 AI 剪辑 / 发布。
