...
const NODE_TYPES = [
  { type: "trigger", label: "触发", icon: "⚡" },
  { type: "skill", label: "Skill", icon: "🧩" },
  { type: "ai", label: "AI 决策", icon: "🤖" },
  { type: "condition", label: "条件", icon: "🔀" },
  { type: "transform", label: "转换", icon: "🔧" },
  { type: "human_approval", label: "人工审批", icon: "🛡️" },
    ...
    <div className="mx-auto max-w-6xl px-6 py-8">
      <div className="flex items-center gap-3">
        <div>
          ...
          <p className="text-xs text-subtle">用一组 Skill 组合成可复用的自动化流程</p>
        </div>
        <div className="ml-auto flex items-center gap-2">
          <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="新工作流名称…" className="w-52" />
          <Button onClick={create}><Plus className="h-4 w-4" /> 新建</Button>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[240px_1fr]">
        ...
        <div className="space-y-2">
          ...
          {workflows.length === 0 && <Card className="p-6 text-center text-xs text-subtle">还没有工作流</Card>}
        </div>
        ...
        {selected ? (
          <div className="space-y-4">
            <Card className="p-4">
              ...
              {selected.schedule && <div className="mt-2 text-[11px] text-subtle">⏰ 定时：{selected.schedule}</div>}
            </Card>

            {/* Node palette */}
            <div className="flex flex-wrap gap-2">
              {NODE_TYPES.map((t) => (
                <button key={t.type} onClick={() => addNode(t.type)} className="rounded-lg border border-border2 bg-surface px-2.5 py-1.5 text-xs text-muted hover:border-accent/50 hover:text-fg">
                  {t.icon} + {t.label}
                </button>
              ))}
            </div>
            ...
            <Card className="overflow-x-auto p-4">
              <div className="flex min-w-max items-stretch gap-3">
                  ...
                  <div key={n.node_key} className="w-60 shrink-0">
                    <div className="rounded-xl border border-border2 bg-surface2 p-3">
                      <div className="flex items-center justify-between">
                        <span className="text-[11px] font-medium text-muted">{nodeTypeMeta(n.type).icon} {nodeTypeMeta(n.type).label}</span>
                      ...
                      {n.type === "human_approval" && (
                        <div className="mt-2">
                          <Input value={String(n.config.message ?? "")} onChange={(e) => updateNodeConfig(n.node_key, { message: e.target.value })} placeholder="审批提示语" className="text-xs" />
                        </div>
                      )}
                      {n.type === "ai" && (
                        <div className="mt-2">
                          <Input value={String(n.config.query ?? "")} onChange={(e) => updateNodeConfig(n.node_key, { query: e.target.value })} placeholder="AI 决策查询" className="text-xs" />
                        </div>
                      )}
                      {n.type === "condition" && (
                        <div className="mt-2 space-y-1.5">
                          <Input value={String(n.config.field ?? "")} onChange={(e) => updateNodeConfig(n.node_key, { field: e.target.value })} placeholder="字段，如 input.count" className="text-xs" />
                          <Input value={String(n.config.value ?? "")} onChange={(e) => updateNodeConfig(n.node_key, { value: e.target.value })} placeholder="比较值" className="text-xs" />
                        </div>
                      )}
                      {n.type === "output" && (
                ...
                {(selected.nodes ?? []).length === 0 && <div className="py-10 text-center text-xs text-subtle">空工作流，从上方添加节点开始</div>}
              </div>
            </Card>

            {/* Run control */}
            <Card className="p-4">
              <SectionTitle>运行参数（JSON）</SectionTitle>
              <div className="flex gap-2">
                <Textarea value={runInput} onChange={(e) => setRunInput(e.target.value)} rows={2} className="code flex-1 text-xs" />
                <Button onClick={() => runWorkflow()} disabled={running} className="self-end">{running ? <Spinner className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5" />} 运行</Button>
              </div>
            </Card>
            ...
            {activeRun && (
              <Card className="p-4">
                <SectionTitle>当前运行 <StatusLabel status={activeRun.status} /></SectionTitle>
                {activeRun.status === "awaiting_approval" ? (
                  <div className="flex items-center gap-3 rounded-lg border border-warn/30 bg-warn/5 p-3 text-xs text-warn">
                    <Pause className="h-4 w-4" /> 等待人工审批
                    <Button size="sm" onClick={() => approveRun(activeRun.id)}>批准继续</Button>
                  </div>
                ) : activeRun.error ? (
              ...
              <Card className="divide-y divide-border">
                {runs.map((r) => (
                  <div key={r.id} className="flex items-center gap-3 px-4 py-2.5 text-xs">
                    ...
                    {r.status === "awaiting_approval" && <Button size="sm" onClick={() => approveRun(r.id)}>批准</Button>}
                  </div>
                ))}
                {runs.length === 0 && <div className="px-4 py-6 text-center text-xs text-subtle">暂无运行记录</div>}
              </Card>
            </div>
          </div>
        ) : (
          <EmptyState icon="🔁" title="选择或创建一个工作流" />
        )}
      </div>
    </div>
  );
...
function nodeTypeMeta(type: string): { label: string; icon: string } {
  const meta = NODE_TYPES.find((t) => t.type === type);
  return meta ?? { label: type, icon: "•" };
}
