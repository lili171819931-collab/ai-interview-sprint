...
/* ── Category board：只显示本分类相关 · 2026 年发布 · 前 30 名 tests ────────── */
...
const seedP = (slug: string, fullName: string, stars: number, cats: string[], created = "2026-01-01"): Project =>
  ({ id: slug, slug, fullName, name: slug, owner: fullName.split("/")[0], tagline: "", description: "", topics: [], language: "TypeScript", license: "MIT", stars, forks: 1, contributors: 2, openIssues: 0, releases: 1, createdAt: created, updatedAt: "2026-08-01", categories: cats, growth7d: 1, growth30d: 2, growth90d: Math.round(stars / 100), profile: FULL_PROFILE, growthHistory: GH } as Project);

const liveR = (fullName: string, stars: number, created = "2026-02-01"): LiveRepo =>
  ({ fullName, name: fullName.split("/")[1], owner: fullName.split("/")[0], stars, forks: 0, openIssues: 0, language: "Python", description: null, topics: ["ai"], createdAt: created, updatedAt: "2026-08-01", homepage: null, license: "MIT" });

// 60 个 agent（2026 发布）+ 3 个 robotics（2026 发布）+ 2024 年发布旧项目 + 50 实时
...
projects.push(seedP("old", "owner/old", 900, ["robotics"], "2024-01-01")); // 2024 发布 → 剔除（不读以前数据）
const liveRepos = Array.from({ length: 50 }, (_, i) => liveR(`owner/live${i}`, 2000 + i));

const bOpp = buildBoardRows({ projects, liveRepos, addedRepos: [], categoryId: "robotics" as any, tab: "opportunity", limit: 30 });
const bStars = buildBoardRows({ projects, liveRepos, addedRepos: [], categoryId: "robotics" as any, tab: "stars", limit: 30 });
const bGrowth = buildBoardRows({ projects, liveRepos, addedRepos: [], categoryId: "robotics" as any, tab: "growth", limit: 30 });

// 只显示本分类相关 2026 发布项目，不做跨分类补齐
assert(bStars.rows.length === 3, "board: 小分类只显示本分类 2026 发布项目（robotics=3，不补 30）");
assert(bOpp.rows.length === 3 && bGrowth.rows.length === 3, "board: 三榜一致只显示 3 条");
assert(bStars.catCount === 3, "board: catCount 为本分类项数");
assert(bStars.rows.every((r) => r.kind === "seed" && ["r1", "r2", "r3"].includes(r.p.slug)), "board: 全部为本分类项目");
assert(!bStars.rows.some((r) => r.kind === "seed" && r.p.slug === "old"), "board: 2024 年发布（以前数据）被剔除");

// agent 大分类：60 个本分类 2026 项目 → 只取前 30
const bAgent = buildBoardRows({ projects, liveRepos, addedRepos: [], categoryId: "agent" as any, tab: "stars", limit: 30 });
assert(bAgent.rows.length === 30, "board: 只显示前 30 名（agent 60 → 30）");
assert(bAgent.rows.every((r) => r.kind === "seed" && r.p.categories.includes("agent")), "board: agent 榜全部为本分类项目");
assert(bAgent.rows.every((r) => !(r.kind === "seed" && r.p.slug.startsWith("r"))), "board: agent 榜不包含 robotics 项目");

// 2026 发布过滤对实时仓库同样生效
const staleLive = liveR("owner/stale-live", 5000, "2024-01-01");
const bLive = buildBoardRows({ projects, liveRepos: [...liveRepos, staleLive], addedRepos: [], categoryId: "robotics" as any, tab: "stars", limit: 30 });
assert(bLive.rows.every((r) => !(r.kind === "live" && r.r.fullName === "owner/stale-live")), "board: 2024 年发布的实时仓库被剔除");

// 去重
const keys = bAgent.rows.map((r) => (r.kind === "seed" ? r.p.fullName : r.r.fullName));
assert(new Set(keys).size === keys.length, "board: 无重复");

// 排序：从高到低
const starVals = bAgent.rows.map((r) => (r.kind === "seed" ? r.p.stars : r.r.stars));
assert(starVals.every((v, i) => i === 0 || starVals[i - 1] >= v), "board: stars 榜从高到低");

// 实时项目并入（loadLive 已按分类查询 → liveTrusted=true）
const bLiveMix = buildBoardRows({ projects, liveRepos: [liveR("owner/robot-live", 800, "2026-01-01")], addedRepos: [], categoryId: "robotics" as any, tab: "stars", limit: 30, liveTrusted: true });
assert(bLiveMix.rows.some((r) => r.kind === "live" && r.r.fullName === "owner/robot-live"), "board: 本分类实时项目并入");
assert(bLiveMix.rows.length === 4, "board: 3 快照 + 1 实时 = 4 条");
// 未加 liveTrusted 时实时项目需按特征猜测
const bGuess = buildBoardRows({ projects, liveRepos: [liveR("owner/random-live", 800, "2026-01-01")], addedRepos: [], categoryId: "robotics" as any, tab: "stars", limit: 30 });
assert(bGuess.rows.every((r) => r.kind === "seed"), "board: 未 trust 时非本分类实时项不并入");
